 ...
 * @version     2.1.0
 */
class WC_Report_Sales_By_Material extends WC_Admin_Report {
	...
	public function get_products_in_category( $category_id ) {
		$term_ids    = get_term_children( $category_id, 'extra_product_template_material' );
		$term_ids[]  = $category_id;
		$product_ids = get_objects_in_term( $term_ids, 'extra_product_template_material' );

		$connected = get_posts( array(
			'connected_type' => 'products_to_product_template',
			'connected_items' => $product_ids,
			'nopaging' => true,
	...
	public function get_chart_legend() {
		...
		foreach ( $this->show_categories as $category ) {
			...
			$category    = get_term( $category, 'extra_product_template_material' );
			$total       = 0;
	...
	public function category_widget() {
		...
		$categories = get_terms( 'extra_product_template_material', array( 'orderby' => 'name' ) );
		?>
		<form method="GET">
			<div>
